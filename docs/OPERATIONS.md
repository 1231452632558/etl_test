...
5. Подключить staging БД к main через временный `postgres_fdw` server и импортировать только snapshot-таблицы.
6. Выполнить общий preflight ключей и схемы до первой записи в main.
7. Выполнить прямой UPSERT из foreign-таблиц отдельными транзакциями, а связанные REPLACE-таблицы — одной группой.
8. После каждой таблицы фиксировать отдельный COMMIT; при ошибке откатывается текущая таблица, уже завершенные таблицы не повторяются в этом запуске.
...
11. Всегда удалить временный FDW-контур; staging удалить только после общего успеха, а при ошибке сохранить для диагностики.
...
Pipeline не выполняет `LOCK TABLE` или advisory-lock сразу для всего списка
snapshot: такая блокировка останавливала весь load из-за одной активно изменяемой
таблицы. PostgreSQL берет необходимые блокировки непосредственно при DML.
Keyed-таблицы фиксируются по одной, а связанная REPLACE-группа остается атомарной.
Перед записью pipeline выводит старые `idle in transaction`, ожидающие Lock и
предыдущие `etl_pipeline:*` соединения. Новые psql-сессии получают диагностический
`application_name` с именем текущей таблицы.
...
- этот срез читается напрямую из staging через `postgres_fdw`
- затем вливается в main через `INSERT ... SELECT ... ON CONFLICT DO UPDATE`
...
- каждая keyed-таблица проверяется и фиксируется своей транзакцией; ошибка откатывает текущую таблицу, но не ранее завершенные
- связанные `snapshot_replace_tables` проверяются и фиксируются одной групповой транзакцией
- ошибка DML до COMMIT откатывает целиком текущую таблицу
...
- в логе есть создание и удаление временного FDW-контура
- выборочно сверены counts и строки нескольких таблиц
