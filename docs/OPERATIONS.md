...
6. Выполнить общий preflight CSV, ключей и схемы до первой записи в main.
7. Выполнить UPSERT отдельными транзакциями по таблицам, а связанные REPLACE-таблицы — одной группой.
8. До каждого COMMIT проверить, что строки staging присутствуют в main с теми же значениями; REPLACE-таблицы проверить по количеству строк.
