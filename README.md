...
3. Подключить staging к main через временный `postgres_fdw`-контур.
4. Влить данные напрямую в стабильную main БД через `UPSERT`.
...
- PostgreSQL-операции
- restore дампа
- прямой перенос staging→main через `postgres_fdw`
- `UPSERT`
- seed ручных таблиц
...
5. FDW-план проходит общий preflight ключей и схемы; затем каждая keyed-таблица применяется отдельной транзакцией, а связанные REPLACE-таблицы — одной группой.
6. `INSERT ... SELECT` читает строки непосредственно из staging; при SQL-ошибке откатывается текущая таблица или REPLACE-группа.
...
Таблицы из `snapshot_replace_tables` не пропускаются: их содержимое читается напрямую
из staging через `postgres_fdw`, а соответствующая main-таблица атомарно заменяется
внутри транзакции. Это режим для таблиц без однозначного ключа,
...
При ошибке ETL staging БД не удаляется, чтобы можно было сопоставить source и main
до следующего запуска. Временный FDW-контур удаляется всегда.
...
- Каждая snapshot-транзакция ограничена `snapshot_batch_timeout_seconds`, а ожидание блокировки — более коротким `snapshot_lock_timeout_seconds`.
- Pipeline не держит одну многомиллионную транзакцию на все snapshot-таблицы: keyed-таблицы фиксируются по одной, а атомарность сохраняется внутри таблицы и связанной REPLACE-группы.
