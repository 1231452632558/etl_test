...
5. Все CSV проходят общий preflight; затем каждая keyed-таблица применяется отдельной транзакцией, а связанные REPLACE-таблицы — одной группой.
6. До каждого COMMIT проверяется, что строки staging присутствуют в main с теми же значениями; при ошибке откатывается текущая таблица или REPLACE-группа.
